...
const WarnGlyph = () => (
  <svg viewBox="0 0 16 16" width="13" height="13" aria-hidden="true">
    <path d="M8 2.2 14.2 13H1.8Z" fill="var(--warn-soft)" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round" />
...
export function TeamRequestCard({
  ...
  const [ticked, setTicked] = useState<Record<number, string[]>>(() =>
    Object.fromEntries(
      item.members.map((m, i) => {
        ...
        return [i, [...new Set(m.connectors ?? [])].filter((c) => allowed.includes(c))];
      }),
    ),
  );
  ...
  const [beyond, setBeyond] = useState<Record<number, string[]>>(() =>
    Object.fromEntries(
      item.members.map((m, i) => {
        ...
        return [
          i,
          [...new Set(m.connectors ?? [])].filter((c) => !offer.includes(c) && other.includes(c)),
        ];
      }),
    ),
  );
  const toggle = (i: number, c: string) =>
    setTicked((cur) => {
      const have = cur[i] ?? [];
      return { ...cur, [i]: have.includes(c) ? have.filter((x) => x !== c) : [...have, c] };
    });
  const addBeyond = (i: number, c: string) => {
    setBeyond((cur) => ({ ...cur, [i]: [...(cur[i] ?? []), c] }));
    setTicked((cur) => ({ ...cur, [i]: [...(cur[i] ?? []).filter((x) => x !== c), c] }));
  };
  ...
  const setForWorkers = (idxs: number[], c: string, on: boolean) => {
    ...
    if (on)
      setBeyond((cur) => {
        const next = { ...cur };
        for (const i of idxs)
          if (!offerFor(item.members[i].persona).includes(c) && !(next[i] ?? []).includes(c))
            next[i] = [...(next[i] ?? []), c];
        return next;
      });
  };
  ...
  const runnable = Array.isArray(item.runnable_models) ? item.runnable_models : null;
  ...
  const serverLabels = Object.fromEntries((runnable ?? []).map((r) => [r.id, r.label]));
  ...
  const [picked, setPicked] = useState<Record<number, string>>(() =>
    Object.fromEntries(
      item.members.map((m, i) => [i, m.resolved_model || m.model || item.lead_model || ""]),
    ),
  );
  ...
  const modelOptions = (persona: string, current: string) => {
    ...
    const others = (runnable ?? []).map((r) => r.id).filter((id) => !recommended.includes(id));
    return (
      <>
        {current !== MIXED && !recommended.includes(current) && !others.includes(current) && (
          <option value={current}>{current ? modelLabelOf(current) : t("team.default_model")}</option>
        )}
  ...
  const warningOf = (i: number) => {
    ...
    return runnable && m.model_warning && (picked[i] ?? "") === (m.resolved_model ?? "") ? m.model_warning : "";
  };
  const memberRow = (i: number): ReactNode => {
    const m = item.members[i];
    const offer = offerFor(m.persona);
    const extra = beyond[i] ?? [];
    const addable = other.filter((c) => !offer.includes(c) && !extra.includes(c));
    ...
    const offList = !!runnable && recommended.length > 0 && !recommended.includes(current);
    return (
        ...
        <span className="teamreq-body">
          <span className="teamreq-who">
            ...
            {runnable ? (
              <>
                ...
                <span className="teamreq-model-pick">
                  <select
                    ...
                    aria-label={t("team.model_for", { who: m.name || m.persona })}
                    title={modelHover(current, labels)}
                    value={current}
                    onChange={(e) => setPicked((cur) => ({ ...cur, [i]: e.target.value }))}
                  >
              ...
              m.model && (
                <span className="teamreq-model" title={modelHover(m.model, labels)}>
                  {" "}
          ...
          {offList && (
            <span className="teamreq-model-note" data-testid={`teamreq-model-note-${i}`}>
          ...
          <span className="teamreq-dials">
            {offer.length > 0 ? (
              <span className="teamreq-connectors">
                <span className="teamreq-dial-label">{t("persona.connectors")}</span>
            ...
            {extra.length > 0 && (
              <span className="teamreq-connectors teamreq-beyond" data-testid={`teamreq-beyond-${i}`}>
                <span className="teamreq-dial-label">{t("team.beyond_worker")}</span>
            ...
            {adding[i] && addable.length > 0 && (
              <span className="teamreq-connectors" data-testid={`teamreq-addlist-${i}`}>
                {addable.map((c) => (
                  <label key={c} className="teamreq-connector">
                    <input
                      type="checkbox"
  ...
  const groupRow = (g: { persona: string; idxs: number[] }): ReactNode => {
    ...
    const tally = [...new Set(models)].map((id) => {
      ...
      return model ? t("team.tally_model", { n: count, model }) : t("team.tally_default", { n: count });
    });
    const tallyText = tally.join(t("team.list_separator"));
    const recommended = runnable ? recommendedFor(persona) : [];
    const offList = !!runnable && uniform && recommended.length > 0 && !recommended.includes(models[0]);
    const warned = idxs.filter((i) => warningOf(i));
    const warn = warned.length
      ? warned.length < n
        ? t("team.warning_partial", { warning: warningOf(warned[0]), have: warned.length, n })
        : warningOf(warned[0])
      : "";
    // Connectors: the role's usual set, then anything beyond it that any worker holds.
    const extra = [...new Set(idxs.flatMap((i) => beyond[i] ?? []))];
    const addable = other.filter((c) => !offer.includes(c) && !extra.includes(c));
    const holders = (c: string) => idxs.filter((i) => (ticked[i] ?? []).includes(c));
    const sharedConnectorReason = (c: string) => {
      const rs = new Set(holders(c).map((i) => item.members[i].connector_reasons?.[c] || ""));
    ...
    const groupBox = (c: string) => {
      ...
      return (
        <label key={c} className="teamreq-connector">
          ...
          <span>{connectorLabel(c)}</span>
          {have > 0 && have < n && (
            <span className="teamreq-partial" data-testid={`teamreq-group-partial-${persona}-${c}`}>
    ...
    const addButton = addable.length > 0 && (
      <button
        ...
        onClick={() => setGroupAdding((cur) => ({ ...cur, [persona]: !cur[persona] }))}
      >
        {t("team.add_another")}
      </button>
    );
    return (
      <div className="teamreq-group" key={persona} data-testid={`teamreq-group-${persona}`}>
          ...
          <span className="teamreq-body">
            <span className="teamreq-who">
                ...
                <>
                  ...
                  <span className="teamreq-model-pick">
                    <select
                      ...
                      title={uniform ? modelHover(models[0], labels) : tallyText}
                      value={uniform ? models[0] : MIXED}
                      onChange={(e) => {
                        const v = e.target.value;
                        if (v !== MIXED) setPicked((cur) => ({ ...cur, ...Object.fromEntries(idxs.map((i) => [i, v])) }));
                      }}
                    >
                      {!uniform && (
              ...
              <button
                ...
                onClick={() => setOpen((cur) => ({ ...cur, [persona]: !cur[persona] }))}
              >
                {isOpen ? t("team.hide_workers") : t("team.show_workers")}
              </button>
            </span>
            {offList && <span className="teamreq-model-note">{t("team.model_off_list_role")}</span>}
            {!uniform && (
              <span className="teamreq-model-note" data-testid={`teamreq-group-tally-${persona}`}>
                {tallyText}
              </span>
            )}
            ...
            <span className="teamreq-dials">
              {offer.length > 0 ? (
                <span className="teamreq-connectors">
                  <span className="teamreq-dial-label">{t("team.connectors_for_all", { n })}</span>
                  {offer.map(groupBox)}
              ...
              {extra.length > 0 && (
                <span className="teamreq-connectors teamreq-beyond">
                  <span className="teamreq-dial-label">{t("team.beyond_role")}</span>
                  {extra.map(groupBox)}
                  {addButton}
                </span>
              )}
        ...
        {isOpen && (
          <div className="teamreq-group-members" data-testid={`teamreq-group-members-${persona}`}>
            {idxs.map(memberRow)}
          </div>
        )}
      </div>
  ...
  return (
    <div className="dirreq-card teamreq-card" data-testid="teamreq-card">
      <div className="teamreq-head">
        ...
        <span className="teamreq-title">
          ...
          {groups.length < item.members.length
            ? t("team.creating_roles", { workers: item.members.length, count: groups.length })
            : t("team.creating", { count: item.members.length })}
        </span>
      </div>
      {note && (
        <div className="teamreq-note-wrap">
          <div className={"teamreq-note" + (noteLong && !noteOpen ? " clamped" : "")}>{note}</div>
          {noteLong && (
            <button
      ...
      {groups.map((g) => (g.idxs.length === 1 ? memberRow(g.idxs[0]) : groupRow(g)))}
      </div>
      {mode && (
      ...
      <div className="dirreq-actions teamreq-actions">
        <span className="spacer" />
        ...
        <button
          ...
          title={t("team.grant_title")}
          onClick={() => onRespond(true, undefined, chat, decisions())}
        >
          {t("team.create_team")}
        </button>
      </div>
