    ...
    <div data-testid="gmail-detail">
      <div className="flex items-center gap-3.5 mb-5">
        ...
        <button
          ...
          title={
            c.managed_paused
              ? t("gmail.setup_below_title")
              : canAdd
                ? ""
                : t("cloud.sign_in_first")
          }
        >
          {c.managed_paused ? t("gmail.add_account_setup_below") : busy ? t("cloud.check_browser") : t("gmail.add_account")}
        </button>
      </div>
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[13px] text-muted"}>
            {t("gmail.setup_blurb")}
            {c.google_client_ready || cloud?.signed_in ? "" : " " + t("gmail.setup_oneclick_hint")}
          </div>
        </div>
      )}
