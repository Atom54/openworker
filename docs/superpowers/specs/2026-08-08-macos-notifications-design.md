...
- **The default backend is a dead end.** `notify-rust`'s macOS default is `mac-notification-sys` → `NSUserNotificationCenter` (deprecated). On macOS 26.6 it *accepts* notifications — `show()` returns `Ok` — and no banner is ever drawn; confirmed by a human watching the screen across several runs. The fix is the `preview-macos-un` feature, which swaps in `UNUserNotificationCenter`.
...
## Do not debug this from the notification database

`~/Library/Group Containers/group.com.apple.usernoted/db2/db` looks like ground truth and is
not. During this work it gave three wrong answers in a row: it recorded notifications that
were never displayed, its `presented` column reads 0 even for Slack notifications the user
demonstrably sees, and it showed **no record at all** for a notification the user watched
appear and clicked. Verify with a human looking at the screen, or with the `[notify]` log
lines in the shell; treat that database as noise.

## Signing

`bundle.macOS.signingIdentity: "-"` makes `codesign` run over the whole bundle so its
signature identifier comes from `Info.plist` (`com.openworker.desktop`). Without it the
bundle keeps the linker's ad-hoc signature, whose identifier is `openworker_desktop-<hash>`,
and `UNUserNotificationCenter` answers every request with `macOS rejected the notification
request` — a hard, observable error, unlike the failures above.

Delivery is intermittent on an ad-hoc signature: the same build notifies sometimes and stays
silent other times, with `show()` returning `Ok` either way. A Developer ID certificate is
the likely cure and is needed for notarised distribution regardless. Untested here.
