...
def build_engine(
    *,
    ...
    # the bundle's (later dirs overwrite earlier in the loader). The CALLABLE keeps
    # _skill_dirs live: repointing Settings ▸ Skills reaches engines already built.
    skill_loader = SkillLoader(
        lambda: [Path(d) for d in (extra_skill_dirs or [])] + _skill_dirs(ws)
    )
